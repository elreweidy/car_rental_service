...
def add_booking():
    ...
    if len(existing_bookings) > 0:
        ...
        for booking in existing_bookings:
            booking_return_date = datetime.datetime.strptime(
                str(booking[4]), "%Y-%m-%d"
            )
            if booking_return_date >= current_date:
                return (
                    jsonify({"message": "Customer already has a current bookings"}),
                    400,
                )
    ...
    return (
        jsonify(
            {"message": "Booking added successfully", "invoice_data": invoice_data}
        ),
        201,
    )
